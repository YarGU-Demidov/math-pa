import { Handler } from 'core/services/message-bus-service/handler';
...
export class HandlersStorage {
	...
	public add(id: number, handler: Function, additionalContext: Object = null): void {
		this.handlers.push(new Handler(id, handler, additionalContext));
	}
	...
	private getHandlerWithId(id: number): Handler {
		const handlers = this.handlers.filter((handlerId) => handlerId.getId() === id);
